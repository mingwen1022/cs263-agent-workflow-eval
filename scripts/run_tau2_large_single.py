"""Run the tau2 retail benchmark with the large single-agent baseline.

The script is intentionally thin: tau2 owns the environment, retail tools,
policy injection, user simulator, conversation loop, and evaluator. This wrapper
only normalizes local paths, Vertex environment variables, config loading, and
result summaries for this project.
"""
...
def write_summary(results: Any, output_dir: Path, metrics: Any, task_ids: list[str]) -> None:
    ...
    simulations = sorted(
        (simulation_summary(sim) for sim in results.simulations),
        key=lambda item: (task_order.get(item["task_id"], len(task_order)), item["trial"]),
    )
    summary = {
...
def configure_tau2_nl_assertion_judge(config: dict[str, Any]) -> None:
    """Force tau2's NL assertion evaluator off its OpenAI default."""
...
def main() -> None:
    ...
    from cs263_agent_eval.tau2.langchain_agent import register_langchain_tau2_agent

    register_langchain_tau2_agent()

    config = load_config(args.config)
    configure_tau2_nl_assertion_judge(config)
    agent_config = config["agent_systems"]["large_single"]
    ...
    if run_name is None:
        ...
        run_name = f"{timestamp}_large_single_{config['domain']}_{len(task_ids)}"
