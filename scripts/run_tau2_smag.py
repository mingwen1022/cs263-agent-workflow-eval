"""Run the tau2 retail benchmark with the SMAG agent."""
...
def configure_tau2_nl_assertion_judge(config: dict[str, Any]) -> None:
    ...
    tau2_config.DEFAULT_LLM_NL_ASSERTIONS = judge_config["llm"]
    tau2_config.DEFAULT_LLM_NL_ASSERTIONS_ARGS = judge_config.get("llm_args", {})
    nl_assertions.DEFAULT_LLM_NL_ASSERTIONS = judge_config["llm"]
    nl_assertions.DEFAULT_LLM_NL_ASSERTIONS_ARGS = judge_config.get("llm_args", {})


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=Path, default=DEFAULT_CONFIG)
    parser.add_argument("--agent-key", default="smag_large")
    ...
    from cs263_agent_eval.tau2.smag_agent import register_smag_agent
    register_smag_agent()
    ...
    results = run_tasks(
        run_config, tasks,
        save_path=save_path, save_dir=output_dir,
